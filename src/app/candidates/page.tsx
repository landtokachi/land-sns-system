      ...
      <div className="space-y-5 max-w-7xl mx-auto">
          ...
          <div className="rounded-2xl overflow-hidden" style={{border:'1px solid #fde68a',background:'linear-gradient(135deg,#fffbeb 0%,#fef3c7 100%)'}}>
            ...
            <div className="p-4 space-y-2">
              {top5.map((c,i)=>{
                ...
                return(
                  <Link key={c.id} href={'/candidates/'+c.id} className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-amber-100 transition-colors group" style={{background:'rgba(255,255,255,0.6)'}}>
        ...
        {view!=='kanban'&&(
          <form className="glass-card p-3 flex flex-wrap gap-2">
            ...
            <select name="status" defaultValue={params.status} className="px-3 py-2 text-sm"><option value="">全ステータス</option>{Object.entries(SS).map(([v,s])=><option key={v} value={v}>{s.label}</option>)}</select>
        ...
        {view==='kanban'&&(
          <div><p className="text-xs text-slate-400 mb-3">ステータス別整理。クリックして詳細・編集。</p>
            <div className="flex gap-4 overflow-x-auto pb-4" style={{minHeight:'60vh'}}>
              {kanban.map(col=>{const vis=col.col?col.items.slice(0,6):col.items;const hid=col.items.length-vis.length;return(
                <div key={col.key} className="flex-shrink-0 w-72 flex flex-col">
                  <div className="flex items-center justify-between px-3 py-2 rounded-t-lg" style={{background:col.hb,borderBottom:'2px solid '+col.bc}}>
                    <span className="text-xs font-bold" style={{color:col.hc}}>{col.label}</span>
                    <span className="text-xs font-semibold px-1.5 py-0.5 rounded-full" style={{background:col.bc,color:col.hc}}>{col.items.length}</span>
                  </div>
        ...
        {!isFiltered&&view==='category'&&(
          <div className="space-y-6">{Object.entries(byCat).map(([cat,items])=>{
        ...
        {!isFiltered&&view==='priority'&&(
          <div className="space-y-8">{Object.entries(byPri).map(([pri,items])=>{
...
function CC({c,now,u}:{c:PostCandidate;now:Date;u:number}) {
  ...
  return(<Link href={'/candidates/'+c.id} className="glass-card block p-4 hover:shadow-md transition-all relative overflow-hidden" style={{borderLeft:'3px solid '+bc}}>
    ...
    <div className="mb-2">
      <div className="flex items-center justify-between mb-0.5"><span className="text-xs text-slate-400">緊急度</span><span className="text-xs font-bold" style={{color:uc}}>{u}pts</span></div>
      <div className="h-1.5 rounded-full bg-slate-100 overflow-hidden"><div className="h-full rounded-full" style={{width:uw+'%',background:uc,opacity:0.8}}/></div>
    </div>
