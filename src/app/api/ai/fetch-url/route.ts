...
◆ 補助金などで複数の枠・ステージ・コース・対象区分がある場合は、それらを1つに要約せず、各区分ごとに「名称・補助率・上限額・対象者・募集締切」を区別して残す（情報を間引かない）。
━━━━━━━━━━━━━━━━━━━━━━
  ...
  "items": [
    {
      ...
      "raw_text": "投稿文作成に使う重要情報を、ページ本文の表記のまま引用した抜粋（1500文字以内）。★補助金で複数の枠・ステージ・コース・対象区分がある場合は、各区分ごとに『名称・補助率・上限額・対象者・募集締切』をすべて漏れなく書き出す（1つにまとめず全区分を残す）。金額・補助率・締切は本文の数値をそのまま。本文に無い情報は書かない",
