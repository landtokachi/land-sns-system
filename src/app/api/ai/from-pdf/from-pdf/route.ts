...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const prompt = `あなたはスタートアップ支援施設「LAND」（北海道十勝・帯広）のSNS担当です。本日: ${today}

以下はPDFから抽出したテキスト（${filename || 'チラシ'}）です。
LANDのSNS投稿候補として登録するための情報を抽出してください。

【LANDのInstagram投稿の傾向（優先度の参考）】
- 補助金・助成金・支援制度 → 最も投稿が多い（特に締切があるもの=高優先）
- 地域のセミナー・イベント → 頻繁に投稿（特に帯広・十勝開催=高優先）
...
以下のJSON形式のみで返してください:
{
  "title": "タイトル（40文字以内）",
  "summary": "要約（300文字以内）",
  "category": "補助金・助成金・支援制度 または イベント・セミナー または LANDの取り組み または とかち財団の取り組み または お知らせ・募集 または コラム・ノウハウ",
  "target_audience": "対象者",
  ...
  "instagram_caption": "Instagram投稿文（150文字以内・絵文字あり）",
  "hashtags": "#補助金 #北海道 など（スペース区切り最大8個）"
}`
