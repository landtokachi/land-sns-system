...
export function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  const inner = (
    <aside
      className={clsx(
        'w-[185px] min-h-screen flex flex-col',
        'fixed inset-y-0 left-0 z-50 transition-transform duration-300',
        'lg:relative lg:translate-x-0',
        isOpen === false ? '-translate-x-full' : 'translate-x-0',
      )}
      style={{
        background: 'linear-gradient(180deg, #0a1628 0%, #0c1c38 50%, #0e1a30 100%)',
        borderRight: '1px solid rgba(255,255,255,0.06)',
        boxShadow: '2px 0 24px rgba(0,0,0,0.5)',
      }}
    >
      {/* ── User profile area (Glazzed style) ── */}
      <div className="px-5 pt-6 pb-4" style={{ borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
        {/* Avatar */}
        <div className="flex flex-col items-center mb-3">
          <div
            className="w-16 h-16 rounded-full mb-3 flex items-center justify-center text-2xl font-black text-white"
            style={{
              background: 'linear-gradient(135deg, #1d4ed8 0%, #7c3aed 50%, #db2777 100%)',
              boxShadow: '0 4px 16px rgba(120,80,200,0.4)',
            }}
          >
            L
          </div>
          <p className="text-sm font-semibold text-white leading-none">LAND スタッフ</p>
          <p className="text-xs mt-1" style={{ color: '#4a6080' }}>Admin Manager</p>
        </div>

        {/* Stats row — Glazzed style */}
        <div className="flex justify-around pt-3" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
          <Link href="/candidates" className="flex flex-col items-center gap-0.5 hover:opacity-80 transition-opacity">
            <span className="text-sm font-bold text-white">投稿候補</span>
            <span className="text-xs" style={{ color: '#4a6080' }}>Candidates</span>
          </Link>
          <div className="w-px" style={{ background: 'rgba(255,255,255,0.07)' }} />
          <Link href="/schedule" className="flex flex-col items-center gap-0.5 hover:opacity-80 transition-opacity">
            <span className="text-sm font-bold text-white">予定</span>
            <span className="text-xs" style={{ color: '#4a6080' }}>Schedule</span>
          </Link>
        </div>
      </div>
      ...
      <nav className="flex-1 px-3 py-3 space-y-0.5 overflow-y-auto">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <span className={clsx('flex-shrink-0', isActive ? 'text-blue-400' : 'text-slate-500 group-hover:text-slate-300')}>
                {item.icon}
              </span>
              <span className={isActive ? 'font-medium text-white' : ''}>{item.label}</span>
              {isActive && <span className="ml-auto w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background: '#3b82f6', boxShadow: '0 0 6px #3b82f6' }} />}
            </Link>
          )
        })}
      </nav>
      ...
      <div className="px-4 py-3 flex items-center gap-2" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
        <div className="w-2 h-2 rounded-full bg-emerald-400 flex-shrink-0" style={{ boxShadow: '0 0 5px #4ade80' }} />
        <span className="text-xs truncate" style={{ color: '#2d4060' }}>公益財団法人とかち財団</span>
      </div>
    </aside>
  )
