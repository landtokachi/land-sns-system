...
export async function generateSocialPosts(candidate: Partial<PostCandidate>): Promise<AiPostResult> {
  ...
  let urlForPost = info.applyUrl || info.sourceUrl || ''
  // 申込・詳細URLが未設定でも、情報源テキスト内にURLがあればそれを拾う（PDF・画像取込でもリンクを載せるため）
  if (!urlForPost && sourceText) {
...
「○○セミナー」は、〜を目的とした（どんな内容・誰向けかを2〜4文で説明）イベントです。〜な方におすすめです。（※冒頭でまず「何のイベントか」を説明してから下記の詳細に進む）
...
★金額・補助率・締切日・開催日・定員・固有名詞（団体名・制度名・人名）は、
　情報源テキストに一字一句そのまま登場する場合のみ使用してよい。
　少しでも記載が無い・曖昧な数値や日付は、書かずに「詳細は公式サイトでご確認ください」で代替する。
　情報源テキストの数値を勝手に四捨五入・概算・言い換えしない。
...
━━━━━━━━━━━━━━━━━━━━━━
【情報量・網羅性ルール（正確性と同じくらい最重要）】

◆ 「嘘を書かない」ことと「情報を省く」ことは別物。情報源テキストに書いてある具体的な事実は、省略せず投稿文にできるだけ多く盛り込む。情報があるのに薄くまとめるのは失敗とみなす。
◆ 情報源にある次の情報は必ず投稿文に反映する：補助率・上限額・補助金額・対象者・募集締切・対象経費・申込方法・会場・日時・枠/ステージ/コースの区分。
◆ 補助金・制度で複数の枠・ステージ・コース・対象区分がある場合は、各区分を見出し付きの箇条書きで「全部」列挙し、それぞれの『対象者・補助率・上限額・締切』を明記する（1つにまとめて薄くしない）。
◆ 情報が豊富なときは短くまとめず、読者が公式サイトを開かなくても概要を把握できるくらい具体的に書く（Instagram本文は長くなってよい）。
◆ ただし情報源に無い数値・条件は依然として書かない（推測禁止）。あくまで「源にある事実を漏れなく書く」のが目的。

★【冒頭の説明（必須）】投稿文は、冒頭の「＼〇〇／」の直後に、まず「これは何の制度・イベント・補助金なのか」を2〜4文で説明してから、日時・会場・対象などの詳細に進む。
　- 説明には情報源テキストにある「目的・概要・特徴・対象・実績」を使う（例：「〜を目指すプログラムです」「過去〜の実績があります」）。
　- いきなり【開催日時】や【対象者】から始めない。読んだ人が「何のための、どんな取り組みか」を最初に理解できるようにする。
　- 説明は情報源にある内容を正確に使い、事実の捏造はしない（書いていない数値・固有名詞は足さない）。

★【詳細リンク（必須）】情報源テキスト（PDF・画像・URLから読み取った本文）に申込フォームや詳細ページのURLがあれば、必ず末尾の「🔗」に正確に転記する（URLを省略・改変しない）。複数あれば申込URLを優先。
━━━━━━━━━━━━━━━━━━━━━━
...
${isSubsidy ? `
【補助金の場合 ― 情報源テキストにある情報を漏れなく記載】
✓ 必ず書く: 補助金の目的・概要、情報源にある金額・補助率・上限額・締切日・対象者
✓ 複数の枠・ステージ・コースがあれば、各区分を箇条書きで全部列挙し、区分ごとに『対象者・補助率・上限額・締切』を書く（例：【A】〇〇ステージ → 対象/補助率/上限/締切、【B】… のように）
...
{
  "instagram_caption": "情報源テキストにある具体的な事実（金額・補助率・上限額・締切・対象者・各枠/ステージ）を漏れなく盛り込んだ、詳しく充実した投稿文。複数の枠・ステージがある場合は各区分を箇条書きで列挙する。情報源に無いことは書かない",
