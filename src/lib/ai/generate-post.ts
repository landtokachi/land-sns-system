...
12. **日付の重複厳禁**: ＜概要＞セクションと本文で同じ日付・情報を二重に書かない。本文に書いたら概要には書かない（または逆）
13. 投稿文末尾に必ず「📌 情報源: [情報元名またはURL]」を記載する（ファクトチェック・確認用）

以下のJSON形式のみで返してください:
{
  "instagram_caption": "Instagramキャプション全文。冒頭＼〇〇〇／から始まり、本文（具体的な数字・金額・日程を豊富に）、＜概要＞セクション（本文と重複しない情報のみ）、URL、締め文、末尾に📌情報源URLまで完全な投稿文。改行あり。800〜1200文字程度。日付・情報の重複なし。",
  "facebook_text": "Facebook投稿文全文。Instagramとほぼ同じ構成でOK。改行あり。URLそのまま記載。末尾に📌情報源URL。日付重複なし。",
